...
class FriendViewSet(viewsets.ViewSet):
    ...
    def find_friends(self,request):
        user=self.request.user.pk
        try:
            request_to_user=FriendRequest.objects.filter(request_from=user,status='Accepted').values()[0]['request_to_id']
            request_from_user=FriendRequest.objects.filter(request_to=user,status='Accepted').values()[0]['request_from_id']
            search_fields=[user,request_to_user,request_from_user]
        except:
            search_fields=[user]
            
        find_friends=User.objects.exclude(id__in=search_fields)
    ...
    def incoming_requests(self,request):
        ...
        if len(incoming_requests)!=0:
            request_from_user=incoming_requests.values()[0]['request_from_id']
            pending=User.objects.filter(id__in=[request_from_user]).values()
    ...
    def sent_requests(self,request):
        ...
        if len(sent_requests)!=0:
            request_to_user=sent_requests.values()[0]['request_to_id']
            pending=User.objects.filter(id__in=[request_to_user]).values()
            serializer = UserSerializer(pending, many=True)
            return Response(serializer.data)
        else:
    ...
    def undo_request(self,request,pk):
        try:
            sent_request= FriendRequest.objects.filter(request_from=self.request.user,status='pending')
            sent_request.delete()
            return Response(status=status.HTTP_204_NO_CONTENT)
        except:
            return Response(status=status.HTTP_400_BAD_REQUEST)
